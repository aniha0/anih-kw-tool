...
_ADD_PAGES = {"📋 キーワード追加", "➕ 商品追加", "📹 動画追加", "📊 追加分析"}
...
# ─── 分析機能 ─────────────────────────────────────────────────────────────────
# 将来追加予定:
#   render_history : 改善履歴（実施日 / 機能 / 変更内容 / 担当）
#   render_compare : Before/After（改善日基準 前後7日: 広告費/売上/ROAS/CVR/CTR/CPC/クリック/注文数）
#   render_result  : 自動評価（改善 / 悪化 / 変化なし）
# ─────────────────────────────────────────────────────────────────────────────

def render_history() -> list:
    """改善履歴一覧を表示し、選択肢リストを返す。
    将来: 実施日 / 機能 / 変更内容 / 担当 をDBまたはCSVから読み込んでテーブル表示。
    戻り値: 履歴レコードのリスト（将来: {"label": str, "date": date, ...}）
    """
    st.caption("（現在データなし）")
    return []   # 将来: 履歴レコードのリストを返す


def render_compare(history_entry) -> None:
    """Before / After 比較ブロック。
    将来: history_entry の日付を基準に前7日・後7日を自動算出して比較表示。
    表示予定: 広告費 / 売上 / ROAS / CVR / CTR / CPC / クリック / 注文数
    """
    st.caption("（現在データなし）")


def render_result(history_entry) -> None:
    """自動評価ブロック。
    将来: 改善 / 悪化 / 変化なし を自動判定して表示。
    """
    st.caption("（現在データなし）")
...
def _render_analysis_page(page_key: str) -> None:
    """分析画面共通レンダラー。
    UI構成: 改善履歴一覧 → 履歴を選択 → 分析開始
    期間は将来 render_compare が選択履歴の日付から前7日・後7日を自動算出する。
    page_key: セッション識別子（"add_analysis" / "cpc_analysis"）
    """
    st.markdown("## 📊 分析")
    ...
    st.markdown("#### 履歴を選択")
    if history_list:
        options = [h["label"] for h in history_list]
        selected_label = st.selectbox(
            "分析する改善履歴を選択してください",
            options,
            key=f"{page_key}_sel",
        )
        selected_entry = history_list[options.index(selected_label)]
        can_run = True
    else:
        st.selectbox(
            "分析する改善履歴を選択してください",
    ...
    if st.session_state.get(f"{page_key}_ran"):
        ...
        render_compare(_entry)

        st.markdown("---")
        st.markdown("##### 評価")
        render_result(_entry)

        st.markdown("---")
        st.caption("今後ここへ分析結果を表示")
