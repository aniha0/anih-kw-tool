...
def page_cpc_unified():
    # CPC調整メニュー統合（キーワード／商品／動画／SB動画）。既存の各CPC調整処理
    # （page_cpc・page_cpc_product・page_cpc_video・SB動画KW用_render_pt_cpc_page呼び出し）を
    # タブとして並べて呼び出すだけで、各処理の中身（判定ロジック・9列表示・CSV12列・
    # 保存処理・分析処理・4週間比較）は一切変更していない。
    _t_kw, _t_prod, _t_video, _t_sbkw = st.tabs(["キーワード", "商品", "動画", "SB動画"])
    with _t_kw:
        page_cpc()
    with _t_prod:
        page_cpc_product()
