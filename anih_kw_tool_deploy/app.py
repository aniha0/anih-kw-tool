...
商品ターゲティングキャンペーン（「商品ターゲ」を含むキャンペーン）のASINが対象です。
動画キャンペーン・オートキャンペーンは除外されます。
...
```
商品ターゲットキャンペーン行を抽出（動画・オート除外）
        ↓
信頼度フィルター適用（注文数≥3 / クリック数≥5 / 広告費≥¥300）
        ↓
採用条件チェック（売上≥売価×2 AND ROAS≥2.0）
        ↓
追加候補として表示
```
...
② ツール画面の上部「CSVアップロード」からファイルを選択する
③ 「🔍 抽出実行」ボタンを押す
④ 商品選択プルダウンから対象商品を選択する
⑤ 一覧テーブルで追加候補ASINを確認する
⑥ CSVをダウンロードする
⑦ Amazon広告管理画面でASINを商品広告へ追加する
...
- キャンペーン名に「商品ターゲ」が含まれていない場合は抽出されません。
- 信頼度フィルター・採用条件の両方を満たすASINのみが対象です。
- 追加はツールで自動実行しません。CSVを確認のうえ手動で登録してください。
...
③ 「🔍 抽出実行」ボタンを押す
④ 商品選択プルダウンから対象商品を選択する
⑤ 一覧テーブルで削除候補ASINを確認する
⑥ CSVをダウンロードする
⑦ Amazon広告管理画面でASINを停止する
...
- 広告費とROASの両方が条件を満たした場合のみ削除候補になります。
- 削除はツールで自動実行しません。CSVを確認のうえ手動で停止してください。
- 一時停止を推奨します。完全削除すると元に戻せません。
...
- 毎週CSVを更新し、削除候補を確認する。
- 削除後は「改善履歴」に登録し、分析機能で広告費削減効果を確認する。
""")
...
動画キャンペーン（「動画」を含むキャンペーン）のASINが対象です。
...
- キャンペーン名に「動画」が含まれていない場合は抽出されません。
- 信頼度フィルター・採用条件の両方を満たすASINのみが対象です。
- 追加はツールで自動実行しません。CSVを確認のうえ手動で登録してください。
...
商品ターゲットキャンペーン行を抽出
        ↓
判定対象条件チェック（広告費≥¥3,000 OR 購入数≥4）
        ↓
満たさない場合 → 判断保留
        ↓
満たす場合 → ランク判定（SS+→SS→S→A→B→C→D→即削除）
        ↓
変更幅を算出（±0・+5・-5・-10円）
        ↓
変更幅≠0のASINのみ詳細テーブルに表示
```
...
② ツール画面の上部「CSVアップロード」からファイルを選択する
③ 「🔍 抽出実行」ボタンを押す
④ 商品選択プルダウンから対象商品を選択（または「全商品」）
⑤ 件数カードでランク別件数を確認する
⑥ 詳細テーブルでランク・変更幅・推奨CPCを確認する（変更幅≠0のみ表示）
⑦ CSVをダウンロードする
⑧ Amazon広告管理画面で各ASINの入札額を変更する
...
```
[商品選択プルダウン: 全商品 / 個別商品名]
[件数カード: SS+ / SS / S / A / B / C / D / 即削除]  ← 全件数表示
[詳細テーブル: ASIN / ROAS / cost / 現在CPC / ランク / 変更幅 / 推奨CPC]
                                                       ← 変更幅≠0のみ
```
...
- 件数カードは変更なし・判断保留を含む全件数を表示します。
- 詳細テーブルは**変更幅≠0のみ**表示します（件数カードとテーブルの件数は一致しません）。
- 即削除ランクはツールで自動停止しません。手動で停止してください。
- CPC変更後は1〜2週間データを蓄積してから再分析してください。
...
- 件数カードは変更なし・判断保留を含む全件数を表示します。
- 詳細テーブルは変更幅≠0のみ表示します（件数カードとテーブルの件数は一致しません）。
- 即削除ランクはツールで自動停止しません。手動で停止してください。
...
広告改善施策（キーワード追加・削除・CPC調整・オート除外KW）を実施した後、
その施策が実際に成果を改善したかどうかを客観的なデータで確認します。
感覚ではなくKPIの数値変化で評価するための機能です。

---

### ■ 分析ロジック

改善履歴の日付を基準として以下の比較を行います。

```
改善履歴の日付
        ↓
Before: 改善日より前のスナップショット（改善前7日に最も近いもの）
After:  改善日より後のスナップショット（改善後7日に最も近いもの）
        ↓
各KPIの差分・増減率を算出
        ↓
自動評価（改善 / 悪化 / 変化なし）
...
開始日・終了日の手動入力は不要です。改善履歴の日付から自動で比較期間を設定します。
...
**③ 広告改善を実施**
広告運用を行います。
例: キーワード追加 / キーワード削除 / CPC調整 / オート除外KW
実施内容は改善履歴へ登録します。
...
**⑨ Before / After確認**
売上 / 広告費 / ROAS / CPC / CTR / CVR / クリック数 / 注文数 を自動比較します。
...
**レポート種別**
- 7日レポート：分析機能で使用するメインのレポートです。毎週保存してください。
- 30日レポート：保存専用です。現在の分析では使用しません。将来の月次分析用に保管します。
...
比較項目: 売上 / 広告費 / ROAS / CPC / CTR / CVR / クリック数 / 注文数 / 表示回数
...
分析済みのレポートを一覧表示します。
以下で絞り込み・検索が可能です。
- 分析対象（改善履歴 / キーワード / 商品 / 動画）
- 評価（改善 / 悪化 / 変化なし）
- 日付範囲
- 対象名（キーワード・ASIN名など）

レポートを開くとBefore / After比較・評価・AIコメントを確認できます。

---

### ■ 7日レポート運用

7日レポートは分析機能で使用するメインのレポートです。
以下すべてに使用します。
- 改善履歴分析
- Before / After比較
- 自動評価
...
毎週の広告改善前後に必ず7日スナップショットを保存してください。
...
30日レポートは保存専用です。
現在の分析機能では使用しません。
将来の月次分析・長期分析・年間分析のために保管します。
毎月1回、30日スナップショットとして保存することを推奨します。
...
① Amazon広告CSV更新
② 7日スナップショット保存
③ 広告改善実施（キーワード追加・削除・CPC調整・オート除外KW）
④ 改善履歴登録
⑤ 7日後に再度CSV更新
⑥ 7日スナップショット保存
⑦ 分析開始・結果確認・レポート保存
...
**毎月の運用**
① 30日レポート保存（月次データ保管用）
※ 30日レポートは現在の分析対象ではありません。将来の月次分析用に保管します。
...
- 分析対象は7日スナップショットのみです。
- 30日レポートは分析対象外です。
- スナップショットは改善前・改善後の両方を保存する必要があります。
- 改善履歴に登録した日付が比較の基準日になります。日付は正確に入力してください。
