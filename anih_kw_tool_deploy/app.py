...
# ===================================================
# 【新規追加】CPC変更履歴（イベント単位・compare_from/compare_to方式）
# ここから4関数はすべて完全新規。既存の_anls_save_cpc_change_history /
# _anls_save_cpc_asin_history / cpc_change_history.json等の既存保存処理・
# 既存JSON・既存分析履歴・既存比較保存機能には一切触れない。
# 新規JSON（cpc_kw_change_events.json / cpc_pt_m_change_events.json /
# cpc_pt_v_change_events.json / cpc_pt_sbv_change_events.json）を
...
def _cpc_change_gen_id() -> str:
    ...
    には一切影響しない、この新機能専用の採番。"""
    return f"{_anls_dt.datetime.now().strftime('%Y%m%d%H%M%S')}_{_anls_random.randint(0, 9999):04d}"


def _cpc_change_current_period():
    """CPC変更を記録する時点の実績期間(ISO開始日・終了日)を、現在の分析対象CSV
    (モジュールレベル変数sf、既存の分析対象選定ロジックの結果をそのまま参照)の
    「期間」列から取得する。既存のrcsv/fcolを呼び出すだけの読み取り専用ヘルパーで、
    sfの選定ロジックや既存のif run:ブロックには一切手を加えない。
    「期間」列が無い/解析できない場合は (None, None) を返す（推測での代替値は作らない）。"""
    try:
        if not sf:
            return None, None
        _df = rcsv(sf)
        ...
        if _starts.notna().any() and _ends.notna().any():
            return _starts.min().date().isoformat(), _ends.max().date().isoformat()
    except Exception:
        pass
    return None, None
...
def _cpc_change_save_event(fname: str, campaign_name, ad_group, target_val, id_col: str,
    ...
    （events配列は使わない）。target_keyはcampaign_name+ad_group+target_valを
    既存のnorm()で正規化して連結する、既存の_cpc_key_fn/_target_key_fnと同じ方式。
    compare_toは記録時点ではNone（30日後比較待ち）。既存の_anls_load/_anls_save
    （無改変）のみを使い、既存JSON・既存関数には一切触れない。"""
    _cn = str(campaign_name or "")
    _ag = str(ad_group or "")
    _tv = str(target_val or "")
    _target_key = f"{norm(_cn)}|{norm(_ag)}|{norm(_tv)}"
    try:
        _before = float(before_cpc or 0)
        _after = float(after_cpc or 0)
        _change_rate = round((_after - _before) / _before * 100, 1) if _before else 0.0
    except Exception:
        _before, _after, _change_rate = 0.0, 0.0, 0.0
    _period_start, _period_end = _cpc_change_current_period()
    _compare_from = {
        "period_start": _period_start, "period_end": _period_end,
        "cost": float(row.get("cost", 0) or 0), "sales": float(row.get("sales", 0) or 0),
        "orders": float(row.get("orders", 0) or 0), "ROAS": float(row.get("ROAS", 0) or 0),
        "impressions": float(row.get("impressions", 0) or 0), "clicks": float(row.get("clicks", 0) or 0),
        "CVR": float(row.get("CVR", 0) or 0),
    }
    _record = {
        "id": _cpc_change_gen_id(),
        "campaign_name": _cn, "ad_group": _ag, id_col: _tv,
        "target_key": _target_key,
        "changed_at": _anls_dt.datetime.now().isoformat(),
        "before_cpc": _before, "after_cpc": _after, "change_rate": _change_rate,
        "compare_from": _compare_from, "compare_to": None,
        "reason": "", "memo": "", "evaluation": "",
    }
...
def _cpc_change_fill_after_comparisons(fname: str, id_col: str) -> None:
    ...
    アップロード処理そのものは呼び出すだけで一切変更しない)の中から、変更日時の
    30日後の日付を含む「期間」のCSVを探し、target_key一致行を集計してcompare_to
    へ格納する。既存のCSVアップロード処理・既存の分析ロジックには一切手を加えない、
    完全新規の独立関数。

    【速度改善のみ・ロジック不変】各CSVは1回だけパースしてDataFrameを保持し、
    その1回分の結果に対して比較待ちイベント全件を突合する（以前はイベントごとに
    毎回rcsvで読み直していた）。判定条件・集計式・比較結果はすべて変更前と同一。"""
    _events = _anls_load(fname)
    _pending = [e for e in _events if not e.get("compare_to")]
    if not _pending:
        return
    _held = []
    for _bk in ("csv_bucket_7d", "csv_bucket_30d", "csv_bucket_other"):
        _held.extend(st.session_state.get(_bk, {}).values())
    if not _held:
        return

    # ① CSVを1回だけパースし、② 期間・突合用列を確定させた状態でDataFrameを保持する。
    _parsed_csvs = []
    for _f in _held:
        try:
            _df_raw = rcsv(_f)
            ...
            if not _pc:
                continue
            _parts = _df_raw[_pc].astype(str).str.split(" - ", expand=True)
            _starts = pd.to_datetime(_parts[0], format="%Y/%m/%d", errors="coerce")
            _ends = pd.to_datetime(_parts[1], format="%Y/%m/%d", errors="coerce") if _parts.shape[1] > 1 else _starts
            if not (_starts.notna().any() and _ends.notna().any()):
                continue
            _p_start, _p_end = _starts.min().date(), _ends.max().date()
            _cc = fcol(_df_raw, ["キャンペーン名", "Campaign Name", "campaign name"])
            _sc = fcol(_df_raw, ["売上", "売上額", "合計売上", "広告費売上高", "7日間の総売上高", "Attributed Sales", "Sales"])
            _oc = fcol(_df_raw, ["合計費用", "費用", "広告費", "コスト", "Cost", "Spend", "spend"])
            _od = fcol(_df_raw, ["商品購入数", "注文数", "注文された商品点数", "Orders", "Purchases"])
            _clk = fcol(_df_raw, ["クリック数", "クリック", "Clicks", "clicks"])
            _imp = fcol(_df_raw, ["インプレッション数", "インプレッション", "Impressions", "impressions"])
            _agn = fcol(_df_raw, ["Ad Group Name", "広告グループ名", "Ad Group", "広告グループ", "ad group"])
            _tkc = fcol(_df_raw, ["ターゲティング", "ターゲッティング", "キーワード", "Targeting", "targeting", "Keyword", "keyword"])
            _kwt = fcol(_df_raw, ["Keyword Text", "Keyword text", "keyword text", "キーワードテキスト"])
            ...
            _df_raw["_cpcchg_tkey"] = _df_raw.apply(
                lambda r: f"{norm(str(r.get(_cc, '') or ''))}|"
                          f"{norm(str(r.get(_agn, '') or '')) if _agn else ''}|"
                          f"{norm(str(r.get('_cpcchg_tv', '') or ''))}",
                axis=1,
            )
            _parsed_csvs.append({
                "df": _df_raw, "p_start": _p_start, "p_end": _p_end,
                "sc": _sc, "oc": _oc, "od": _od, "clk": _clk, "imp": _imp,
            })
        except Exception:
            continue
    if not _parsed_csvs:
...
def page_cpc_change_history():
    """📝 CPC変更履歴 — 完全新規の独立ページ。既存のpage_anls_history()・
    _anls_render_saved_report()・既存JSON(cpc_change_history.json、
    anls_cpc_kw.json等)・既存の比較保存機能には一切触れず、新規JSON・新規関数
    のみで完結する。既存の分析履歴とは完全に分離して管理する。"""
    st.markdown("### 📝 CPC変更履歴")
    st.caption("CPC変更を記録したイベントを、30日後の実績と比較して表示します（既存の分析履歴とは別管理です）。")
    _targets = [
    ...
    if not _all_events:
        ...
        return
    _cat_options = ["全て"] + [t[1] for t in _targets]
    _status_options = ["全て", "比較待ち", "比較完了"]
    _fc1, _fc2, _fc3 = st.columns(3)
    with _fc1:
        _sel_cat = st.selectbox("対象種別", _cat_options, key="_cpc_change_hist_cat")
    with _fc2:
        _sel_status = st.selectbox("状態", _status_options, key="_cpc_change_hist_status")
    with _fc3:
    ...
    for _ev, _label, _id_col, _fname in _all_events:
        ...
        if _sel_status != "全て" and _st_now != _sel_status:
            continue
    ...
    for _ev, _label, _id_col, _fname in _filtered[:50]:
        ...
        with st.expander(f"{_st_now}　{_label}　{_tv}　{_changed_disp}", expanded=False):
            st.markdown(
                f"**キャンペーン**：{_ev.get('campaign_name','―')}　｜　"
                f"**広告グループ**：{_ev.get('ad_group','―')}　｜　**{_label}対象**：{_tv}"
            )
            st.markdown(f"**変更日**：{_changed_disp}")
            st.markdown(
                f"**CPC**：{_fmt_yen(_ev.get('before_cpc'))} → {_fmt_yen(_ev.get('after_cpc'))}"
                f"　（{float(_ev.get('change_rate', 0) or 0):.1f}%）"
            )
            st.markdown("---")
            _cf = _ev.get("compare_from") or {}
            _ct = _ev.get("compare_to") or {}
            _p1 = f"{_cf.get('period_start','―')}〜{_cf.get('period_end','―')}" if _cf else "―"
            _p2 = f"{_ct.get('period_start','―')}〜{_ct.get('period_end','―')}" if _ct else "―（比較待ち）"
            _tbl = pd.DataFrame(
                [
                    ["広告費", _fmt_yen(_cf.get("cost")), _fmt_yen(_ct.get("cost")) if _ct else "―"],
                    ["売上", _fmt_yen(_cf.get("sales")), _fmt_yen(_ct.get("sales")) if _ct else "―"],
                    ["注文数", _fmt_num(_cf.get("orders")), _fmt_num(_ct.get("orders")) if _ct else "―"],
                    ["ROAS", _fmt_roas(_cf.get("ROAS")), _fmt_roas(_ct.get("ROAS")) if _ct else "―"],
                    ["インプレッション", _fmt_num(_cf.get("impressions")), _fmt_num(_ct.get("impressions")) if _ct else "―"],
                    ["クリック", _fmt_num(_cf.get("clicks")), _fmt_num(_ct.get("clicks")) if _ct else "―"],
                    ["CVR", _fmt_pct(_cf.get("CVR")), _fmt_pct(_ct.get("CVR")) if _ct else "―"],
                ],
                columns=["指標", f"比較元（{_p1}）", f"比較先（{_p2}）"],
            )
            st.table(_tbl)
            st.markdown("---")
...
def _render_pt_cpc_page(dc_pt, page_title: str, sel_key: str, hist_fname: str = "", id_col: str = "asin", csv_disp_only: bool = False, select_label: str = None, change_log_fname: str = ""):
...
def page_cpc_product():
    ...
    with _t1:
        _render_pt_cpc_page(dc_cpc_product, "商品CPC調整", "cpc_product_sel", "cpc_pt_m_history.json", change_log_fname="cpc_pt_m_change_events.json")
    with _t2:
...
def page_cpc_video():
    _t1, _t2 = st.tabs(["CPC調整", "分析"])
    with _t1:
        _render_pt_cpc_page(dc_cpc_video, "動画CPC調整", "cpc_video_sel", "cpc_pt_v_history.json", select_label="動画商品", change_log_fname="cpc_pt_v_change_events.json")
    with _t2:
        # KW版と同一仕様の「対象ごとの個別4週間比較タブ＋状態アイコン」。
...
def page_cpc_sbvideo():
    ...
    _t1, _t2 = st.tabs(["CPC調整", "分析"])
    with _t1:
        _render_pt_cpc_page(dc_cpc_video_kw, "SB動画KWターゲCPC調整", "cpc_video_kw_sel", id_col="keyword", csv_disp_only=True, select_label="動画KW", change_log_fname="cpc_pt_sbv_change_events.json")
    with _t2:
