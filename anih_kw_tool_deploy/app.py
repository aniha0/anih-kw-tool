...
オートキャンペーンの検索語句から成果が出たKWを抽出し、マニュアルキャンペーン（部分一致）への追加候補を表示します。

---

**集計元データ**

検索用語レポートのキャンペーン名に「オート」または「auto」を含む行の「検索用語（カスタマーの検索用語）」列。

---

**抽出条件（すべてを同時に満たす場合に追加候補）**
...
| 除外対象 | 内容 |
|---|---|
| マニュアルKWと完全一致する語句 | ターゲティング列の登録済みKWと一致するため除外 |
| マニュアルKWに部分包含される語句 | マニュアルに2語以上で含まれる語句は除外 |
| ブランドワード | アニハ・ANIHA・ゾイック・ノルバサン・マラセブを含む語句 |
| 商品コード | ASIN形式（B0始まり10文字）・8桁以上の数字・英数字のみの語句 |
| タイトル文字列 | 「」（）などの括弧を含む / 20文字以上 / スペース3個以上の語句 |
| 同一意図KW | 語順・表記ゆれが同じKWは代表1件に集約 |
| 未分類キャンペーン | 売価マスタ（PRICES辞書）に未登録の商品は除外 |
...
| 表示エリア | 内容 |
|---|---|
| キャンペーンフィルター | 全キャンペーン＋各キャンペーン |
| 件数バッジ | 該当件数 N件 |
| KW一覧（コピー用） | ROAS降順。Amazon広告コンソールへそのままペースト可能 |
| 詳細テーブル | keyword / キャンペーン名 / ROAS / 売上 / 広告費 / 注文数 / クリック数 |
""")
...
商品ターゲティングキャンペーン（マニュアル）で成果が出ているASINを追加候補として表示します。

---

**集計元データ**

キャンペーン名に「商品ターゲ」を含み、「動画」を含まず、「オート」「auto」を含まない行のターゲティング列からASIN（B0[A-Z0-9]{8}）を抽出し、ASIN単位で集計。
...
| 除外対象 | 内容 |
|---|---|
| ASINが抽出できない行 | ターゲティング列にB0始まり10文字のASINが存在しない行 |
| 未分類キャンペーン | 売価マスタ未登録の商品 |
...
| 詳細テーブル | ASIN / キャンペーン名 / 広告グループ / 注文数 / クリック数 / 広告費 / 売上 / ROAS / 採用理由 |
| CSVダウンロード | 商品追加_{商品名}.csv |
""")
...
動画キャンペーンで成果が出ているASINを追加候補として表示します。

---

**集計元データ**

キャンペーン名に「商品ターゲ」かつ「動画」を含む行のターゲティング列からASIN（B0[A-Z0-9]{8}）を抽出し、ASIN単位で集計。

---

**抽出条件**

商品追加と同一（信頼度フィルター＋採用条件）。
...
マニュアルキャンペーンで利益を毀損している検索語句を停止候補として表示します。完全一致で除外登録することを推奨します。

---

**集計元データ**

検索用語レポートのマニュアルキャンペーン（キャンペーン名に「オート」「auto」を含まない）行の「検索用語（カスタマーの検索用語）」列。ASIN形式・asin:形式・category:形式の語句は集計前に除外済み。
...
| 除外対象 | 内容 |
|---|---|
| 勝ちKW | キーワード追加で抽出された採用KWは誤削除防止のため除外 |
| 未分類キャンペーン | 売価マスタ未登録の商品 |
...
| キャンペーンフィルター | 全キャンペーン＋各キャンペーン |
| 件数バッジ | 削除対象件数 N件 |
| KW一覧（コピー用） | Amazon広告コンソールへそのままペースト可能 |
| 詳細テーブル | keyword / キャンペーン名 / ROAS / 広告費 / 売上 |
""")
...
商品ターゲティングキャンペーン（マニュアル）で利益を毀損しているASINを停止候補として表示します。

---

**集計元データ**

キャンペーン名に「商品ターゲ」を含み、「動画」を含まず、「オート」「auto」を含まない行のターゲティング列からASINを抽出し、ASIN単位で集計。
...
| 詳細テーブル | ASIN / キャンペーン名 / 広告グループ / 広告費 / 売上 / ROAS / 削除理由 |
| CSVダウンロード | 商品削除_{商品名}.csv |
""")
...
動画キャンペーンで利益を毀損しているASINを停止候補として表示します。

---

**集計元データ**

キャンペーン名に「商品ターゲ」かつ「動画」を含む行のターゲティング列からASINを抽出し、ASIN単位で集計。

---

**削除条件**

商品削除と同一（広告費 ≥ 売価×2 かつ ROAS < 0.8）。
...
SP広告マニュアルキャンペーンのキーワードの入札額（CPC）を最適化します。ランクを判定してCPC変更幅を提示します。

---

**集計元データ**

キャンペーン名が「SP広告.*マニュアル」または「SP.*manual」にマッチし、「商品ターゲ」「動画ターゲ」を含まない行。Keyword Text列を優先（存在しない場合はターゲティング列）でKWテキスト単位に集計。
...
| C | 1.5 ≤ ROAS < 1.8 | −5円 |
| D | ROAS < 1.5 | −10円 |
| 即削除 | ROAS < 0.8 かつ 広告費 ≥ 閾値 | — |

即削除閾値: 売価 ≤¥1,500 → ¥3,000 ／ 売価 ≤¥2,000 → ¥4,000 ／ 売価 >¥2,000 → ¥5,000

現状維持（±0円）: SS（購入数≥20 かつ 2.0≤ROAS<4.0）/ A（3.0≤ROAS<4.0）/ B（1.8≤ROAS<3.0）
...
| 除外対象 | 内容 |
|---|---|
| オートキャンペーン行 | キャンペーン名に「オート」「auto」を含む行 |
| 商品ターゲ行 | KWテキストがASIN・asin:・category:・complement・substituteを含む行 |
| Keyword Text空欄行 | KWテキストが取得できない行 |
| ブランドKW | アニハ・あには・アニは を含むKW |
| 判断保留 | 広告費 < ¥3,000 かつ 購入数 < 4件（変更なし） |
| 未分類キャンペーン | 売価マスタ未登録の商品 |
...
| 表示エリア | 内容 |
|---|---|
| キャンペーンフィルター | 全商品＋各キャンペーン |
| ランク件数カード | SS+ / SS / S / A / B / C / D / 即削除（全件カウント） |
| 本日調整対象 | CPC上げ件数 / CPC下げ件数 / 変更対象合計 |
| 詳細テーブル | **変更幅≠0のみ表示**（KWテキスト / 広告グループ / ROAS / 広告費 / 売上 / 購入数 / 現在CPC / 判定ランク / 推奨アクション / 変更幅 / 推奨CPC） |
| CSVダウンロード | 実行用（変更幅≠0件のみ）と全件の2種類 |
""")
...
商品ターゲティングキャンペーン（マニュアル）のASINターゲの入札額を最適化します。
...
キーワードCPC調整と同一（SS+: 購入数≥20 かつ ROAS≥4.0 / S: ROAS≥4.0）。

---

**CPCを下げる条件**

キーワードCPC調整と同一（C: −5円 / D: −10円 / 即削除）。

現状維持（±0円）: SS / A / B — キーワードCPC調整と同一。
...
| ランク件数カード | SS+ / SS / S / A / B / C / D / 即削除（全件カウント） |
| 本日調整対象 | CPC上げ件数 / CPC下げ件数 / 変更対象合計 |
| 詳細テーブル | **変更幅≠0のみ表示**（ASIN / 広告グループ / ROAS / 広告費 / 売上 / 購入数 / 現在CPC / 判定ランク / 推奨アクション / 変更幅 / 推奨CPC） |
| CSVダウンロード | 全件出力（変更幅±0を含む）1種類 |
""")
...
商品CPC調整と同一（SS+: +5円 / S: +5円 / C: −5円 / D: −10円 / 即削除）。
