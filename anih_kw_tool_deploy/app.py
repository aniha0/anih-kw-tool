...
オート広告（自動ターゲティング）で利益が確認できた検索語句を検出し、追加候補として
手動広告（部分一致）のマニュアルキャンペーンへ「キーワード」として登録するためのリストを表示します。

サイドバーの「➕ キーワード追加」内に、キーワードのボタンがあります。
...
内容によって振り分け、キーワード形式のみをこのページで扱います。

| 検索用語の内容 | 振り分け先 |
|---|---|
| 通常の検索語句（日本語など） | 📋 キーワード追加 |
| ASIN形式（例: B0XXXXXXXXX） | 対象外（商品ページで扱う） |
| category形式（例: category:〜） | 対象外（動画ページで扱う） |

既存マニュアルキャンペーンに登録済みの語句（完全一致・部分包含）はあらかじめ除外されます。

---

**採用条件（すべてを同時に満たす場合に追加候補）**
...
| 除外対象 | 内容 |
|---|---|
| マニュアル広告に完全一致登録済みの語句 | 重複のため除外 |
| マニュアル広告に部分一致登録済みの語句 | 包含関係のため除外 |
| ブランドワード（アニハ・ANIHA など） | ブランドKWのため除外 |
| 商品コード・長文タイトル | 品質基準外のため除外 |
...
| 詳細テーブル | keyword / キャンペーン / ROAS / 売上 / 広告費 / 注文数 / クリック数 |

各ページに「キャンペーン」フィルター（全キャンペーン＋各キャンペーン）があり、
件数バッジ・追加候補KW一覧（コピー用）・詳細テーブルを表示します。
""")
...
マニュアル広告（手動広告）で利益を毀損している検索語句を検出し、停止候補として表示します。
完全一致で除外登録（ネガティブKW）することを推奨します。

サイドバーの「🚫 キーワード削除」内に、キーワードのボタンがあります。

---

**集計の元データ**

「検索用語レポート」内のマニュアルキャンペーン行のみを対象にします。
オートキャンペーン由来の行は除外されます（オート除外KWページで別途対応）。

| 対象 | 内容 |
|---|---|
| 含む | マニュアルキャンペーン由来の検索語句 |
| 除外 | オートキャンペーン由来の語句 |
| 除外 | ASIN形式・category形式の語句 |
...
| 除外対象 | 内容 |
|---|---|
| 勝ちKW（キーワード追加候補） | 追加対象KWを誤って削除しないための保護 |
| 未分類キャンペーン | 売価が特定できないため除外 |
...
| 詳細テーブル | keyword / キャンペーン / ROAS / 広告費 / 売上 |

各ページに「キャンペーン」フィルター（全キャンペーン＋各キャンペーン）があり、
件数バッジ・削除対象KW一覧（コピー用）・詳細テーブルを表示します。
""")
...
既存マニュアルKWの入札額（CPC）を最適化します。
成果に応じてランクを判定し、CPC上げ・現状維持・CPC下げ・即削除のいずれかを推奨します。

サイドバーの「📈 CPC調整」内に、キーワードのボタンがあります。

---

**集計の元データ**

「検索用語レポート」内のSP広告（マニュアル）キャンペーン行のみを対象にします。
Keyword Text列を優先してKWテキスト単位で集約します。

| 対象 | 内容 |
|---|---|
| 含む | SP広告マニュアルキャンペーン（Keyword Text単位） |
| 除外 | オートキャンペーン由来の行 |
| 除外 | 商品ターゲ・動画ターゲ行（ASIN / category形式） |
| 除外 | ブランドKW（アニハ・あには・アニは） |
...
| 除外対象 | 内容 |
|---|---|
| オートキャンペーン行 | CPC調整対象外（オート除外KWページで別途対応） |
| 商品ターゲ・動画ターゲ行 | KW以外のターゲティング形式のため除外 |
| Keyword Text空欄行 | KWテキストが取得できない行を除外 |
| 未分類キャンペーン | 売価が特定できないため除外 |
...
| ランク件数カード | SS+ / SS / S / A / B / C / D / 即削除（全件） |
| 本日調整対象 | CPC上げ件数 / CPC下げ件数 / 変更対象合計 |
| 詳細テーブル | **変更幅 ≠ 0円のみ**（CPC上げ・CPC下げ対象のみ表示） |
| CSVダウンロード | 実行用（変更対象のみ）と全件の2種類 |

各ページに「キャンペーン」フィルター（全商品＋各キャンペーン）があり、
ランクカード・本日調整対象・詳細テーブル・CSVダウンロードを表示します。
""")
