...
# ─── CSV管理（新規追加・UI/保持専用。既存の分析・判定・計算ロジックには一切関与しない）───
def _csv_bucket_uploader(label: str, state_key: str, widget_key: str):
    """複数CSVを1回の選択で読み込み、かつ過去に読み込んだCSVを保持し続ける
    ためのUI専用ヘルパー（新規追加）。ファイル名をキーにst.session_state内の
    辞書へ蓄積するだけで、既存の分析関数・sf変数・抽出実行ロジックには
    一切触れない（読み込んだファイルはここで保持されるのみで、どの既存処理
    からも参照されない）。"""
    st.markdown(f"**{label}**")
    _new_files = st.file_uploader(label, type="csv", accept_multiple_files=True,
...
# ─── File Upload ────────────────────────────────────
# 【重要】既存の「抽出実行」ボタン以下の全ロジックは、単一ファイル変数sf
# （key="sf", on_change=clear）を必須の入力として設計されている。今回の
# 依頼は「CSVアップロードUI・CSV管理部分のみ変更」「既存ロジック・関数名・
# 引数・データ構造は変更禁止」「既存機能を壊すリファクタリング禁止」のため、
# 「基準レポートCSV」欄はsf変数・on_change=clearとも従来のまま維持し、
# 新規の3分類（7日比較・30日比較・その他）は保持専用の別系統として追加した。
# ※ この設計判断（基準CSV欄を残す）は、既存の抽出実行ロジックを壊さない
# ために必要と判断したものであり、ご意図と異なる場合はご指示ください。
...
with _u1:
    ...
    st.markdown("")
    _csv_bucket_uploader("📊 その他（日数）CSV", "csv_bucket_other", "csv_bucket_other_uploader")
    st.markdown("")
    st.markdown("**📊 基準レポートCSV**")
    sf = st.file_uploader("基準レポートCSV", type="csv", key="sf", on_change=clear, label_visibility="collapsed")
...
| 項目 | 内容 |
|---|---|
| レポート名 | Amazon検索用語レポート（画面上部の「📊 Amazon検索用語レポート」欄） |
...
1. 画面上部の「📊 Amazon検索用語レポート」にCSVをアップロードする
2. 「🔍 抽出実行」ボタンを押す（サイドバーの各機能に共通する最初の処理です）
...
| 項目 | 内容 |
|---|---|
| レポート名 | Amazon検索用語レポート（画面上部の「📊 Amazon検索用語レポート」欄。キーワード追加など他機能と共通） |
| 必須列 | 検索用語（カスタマーの検索用語）／キャンペーン名／売上／広告費／ターゲティング |
...
| 項目 | 内容 |
|---|---|
| レポート名 | Amazon検索用語レポート（画面上部の「📊 Amazon検索用語レポート」欄。キーワード追加・削除と共通） |
...
| 項目 | 内容 |
|---|---|
| レポート名 | Amazon検索用語レポート（画面上部の「📊 Amazon検索用語レポート」欄。キーワード追加・削除・CPC調整と共通） |
| 必須列 | 検索用語（カスタマーの検索用語）／キャンペーン名／売上／広告費／ターゲティング |
...
有力キーワード候補をスコアリングし、TOP10として抽出します。アプリ上部の「Amazon検索用語レポート」を
...
- この機能はアプリ上部の「Amazon検索用語レポート」「🔍 抽出実行」とは独立した別系統の処理です
  （抽出実行を行っていなくても利用できます）
...
1. 画面上部の「📊 Amazon検索用語レポート」にCSVをアップロードする（他機能と共通）
2. 「🔍 抽出実行」ボタンを押す
3. サイドバーの「📥 ダウンロード」を開く
...
- キーワードCPC調整ZIPは全件出力です。「キーワードCPC調整」タブの詳細テーブル（変更幅が±0円ではない行のみ表示）
  とは収録件数が異なります
- このページは画面上部の「Amazon検索用語レポート」「🔍 抽出実行」の結果を元にしています。抽出を実行していない
  場合、いずれのボタンも表示されません
...
**Q. ダウンロードボタンが表示されない**
A. 対象データが0件の場合、ボタンごと非表示になります。まず画面上部でレポートをアップロードし
「🔍 抽出実行」を押してください。
